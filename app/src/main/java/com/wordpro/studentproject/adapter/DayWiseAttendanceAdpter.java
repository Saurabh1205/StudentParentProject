...
public class DayWiseAttendanceAdpter extends RecyclerView.Adapter<DayWiseAttendanceAdpter.ViewHolder>{

    Context Mcontext;
    public static DaywiseAttndModel daywiseAttndModel;
    public static ArrayList<DaywiseAttndModel.DataBean> daywiseArrayList;

    public DayWiseAttendanceAdpter(Context context, ArrayList<DaywiseAttndModel.DataBean> pendingJobDetails) {
    ...
    @NonNull
    @Override
    public DayWiseAttendanceAdpter.ViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int postion) {
        View itemView = LayoutInflater.from(viewGroup.getContext())
                .inflate(R.layout.day_wise_all_list, viewGroup, false);
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int postion) {
        ...
        if (URLEndPoints.ConstanceLoginUser.equalsIgnoreCase("Student")){
            holder.txt_Theory.setTextColor(ContextCompat.getColor(Mcontext, R.color.text_blue));
            holder.txt_Regular.setTextColor(ContextCompat.getColor(Mcontext, R.color.text_blue));
            holder.txt_date.setTextColor(ContextCompat.getColor(Mcontext, R.color.text_blue));
        }else {
            holder.txt_Theory.setTextColor(ContextCompat.getColor(Mcontext, R.color.present));
            holder.txt_Regular.setTextColor(ContextCompat.getColor(Mcontext, R.color.present));
            holder.txt_date.setTextColor(ContextCompat.getColor(Mcontext, R.color.present));
        }
        holder.userName.setText(daywiseArrayList.get(postion).getPROF_EMP_FN_MN_SHORT());
        holder.txt_name.setText(daywiseArrayList.get(postion).getSUBJECT_DESCRIPTION());
...
*/
        ...
        /* DateFormat f = new SimpleDateFormat("MM/dd/yyyy hh:mm:ss a");
         ...
         try {
             show = formatter1.parse(dateStr);
             String v = show.toString();

             holder.txt_date.setText(v);
         }catch (Exception e){
             e.printStackTrace();
         }
*/
